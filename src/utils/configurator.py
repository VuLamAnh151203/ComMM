    ...
    def _load_dataset_model_config(self, config_dict, mg):
        ...
        for file in file_list:
            if not os.path.isfile(file):
                raise FileNotFoundError('Config file does not exist: {}'.format(file))
        ...
        # Paths from YAML are relative to ``src``, regardless of the caller's
        # working directory. Explicit paths supplied by callers remain relative
        # to their current working directory and override this value later.
        for path_key in ('data_path', 'checkpoint_dir', 'recommend_topk'):
            configured_path = file_config_dict.get(path_key)
            if configured_path and not os.path.isabs(configured_path):
                file_config_dict[path_key] = os.path.abspath(
                    os.path.join(cur_dir, '..', configured_path)
                )
